          ...
          <div className="content">
            <div className={auto ? '' : 'clear-border'}>
              <Radio.Group 
                ...
                buttonStyle="solid"
              >
                <Radio.Button style={{width: buttonWidth, textAlign: "center"}} value="day"><FormattedMessage id="order.day" defaultMessage="Yesterday" /></Radio.Button>
                <Radio.Button style={{width: buttonWidth, textAlign: "center"}} value="week"><FormattedMessage id="order.week" defaultMessage="Last 7 days" /></Radio.Button>
                <Radio.Button style={{width: buttonWidth, textAlign: "center"}} value="month"><FormattedMessage id="order.month" defaultMessage="Last 30 days" /></Radio.Button>
                {
                    auto &&
                    <Radio.Button style={{width: buttonWidth, textAlign: "center"}} value="auto"><FormattedMessage id="order.customize" defaultMessage="Customize" /></Radio.Button>
                }
              </Radio.Group>
              {
                  !auto &&
                ...
                <div className="right-search">
                 <span className={classes.navSpan}><FormattedMessage id="pay.source" defaultMessage="Source" />：</span>
                <Select
                value={source}
                ...
                >
                <Select.Option value="all" ><FormattedMessage id="pay.allSystems" defaultMessage="All Systems" /></Select.Option>
                  {
                    statBySystem.map((item: any, index: number) => {
                      return <Select.Option key={index} value={item.english} >{locale === 'zh'? item.label : item.english}</Select.Option>
                    })
                  }
              </Select>
              </div>
        ...
        <div className={classes.loopCharts}>
          <div className="panel-wrap">
            <div className="title-bar">
              <h3 className="title"><FormattedMessage id="pay.paymentMethodAnalysis" defaultMessage="Payment Methods Analysis" /></h3>
              <div className="right-search">
                <span><FormattedMessage id="pay.source" defaultMessage="Source" />：</span>
                <Select
                value={source1}
                style={locale === 'zh'?{width: 190}:{width: 120}}
                className={classes.link} 
            ...
            <div className="total-money"><FormattedMessage id="pay.totalAmount" defaultMessage="Total Amount(yuan)" /> ：{this.formatNum(typeTotal.toFixed(2))}</div>
            <div className="content">
          ...
          <div className="panel-wrap">
            <div className="title-bar">
              <h3 className="title"><FormattedMessage id="pay.percentageOfPayment" defaultMessage="Payment Percentage" /></h3>
            </div>
            <div className="total-money"><FormattedMessage id="pay.totalAmount" defaultMessage="Total Amount(¥)" /> ：{this.formatNum(moneyTotal.toFixed(2))}</div>
            <div className="content">
...
const mapState2Props = ({
  pay: { chartData, PayMethod, statBySystem},
  intl: { locale }
}: any) => ({
  chartData,
  PayMethod,
...
const mapDispatch2Props = ({
  ...
  getList, payDetail, paymentAmounts, getStatBySystem, getPayMethod });
