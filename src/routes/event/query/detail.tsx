      ...
      [
        {
          ...
          value: (
            <Tooltip
              placement="topLeft"
              trigger="click"
              title={newData.map((item: any) => item).length ? newData[0].targets : null}
            >
              <span className={`${classes.col}`} style={{ maxWidth: "900px" }}>
                {newData.map((item: any) => item).length ? newData[0].targets : null}
              </span>
            </Tooltip>
          ),
          colSpan: "5"
...
const mapDispatch2Props = ({
  event: { queryDetail, getEventLevelList,getEventTypeList,getAppCodeList, getEventStatusList, setDetail }
}: any) => ({
  queryDetail,
  getAppCodeList,
  getEventLevelList,
