...
import { Radio, DatePicker, Icon, Spin, message } from 'antd';
...
class User extends React.Component<EventAnalyze, any> {
  ...
  render() {
    const { endOpen, time, startTime, endTime, dataSelect, } = this.state;
    const {
      classes,
      analyseCustom,
      analyseLoop,
    ...
    const dataList = analyseCustom && analyseCustom.data;
    const successTotal = dataList && dataList.byStatus.find((item: any) => item.code === 4).total
    const totals = analyseCustom && analyseCustom.total;
    const loopList = loopData ? loopData.map((item: any) => { return item }) : (dataList && dataList.byTypeId)
    const totalLoop = analyseLoop.total ? analyseLoop.total : totals
    const Time = time ? (time === 'auto' ? 4 : time) : 1
    const customStart = dataList && dataList.startTime;
    const customEnd = dataList && dataList.endTime;
    return (
      <div className={classes.root}>
        <Spin tip="Loading..." spinning={false}>
          ...
          <div className={classes.overview}>
            ...
            <div className="right">
              {
                dataList && dataList.byLevel.map((item: any, index: number) => {
                  ...
                  >
                    <div className="item" onClick={this.eventNav}>
                      ...
                      <div className="value" style={{ borderRight: "1px solid #E9E9E9" }}>{format(item.count)}</div>
                    </div>
                  </Link>
                })
              }
          ...
          <div className="panel-wrap" style={{ display: "flex", height: "350px" }}>
            <div style={{ width: "50%" }}>
              ...
              <div className="content" >
                {dataList && dataList.byType.length !== 0 ?
                  (dataList && dataList.byType.map((item: any, index: number) => {
                    let _style: any = {
              ...
              <div className="content">
                <LoopCharts data={loopList}
                  ...
                  colorCallback={(label: string) => {
                    const item = loopData && loopData.find((item: any) => item.label === label);
          ...
          <div className={classes.loopCharts}>
            <div className="panel-wrap minLoops">
              ...
              <div style={{ display: "flex" }}>
                <div className="content" style={{ width: "25%", height: "235px" }}>
                  <MinLoopCharts
                    dataLink={dataList && dataList.byStatus[0]}
                    content={
                      `<div style="color:#666666;font-size:16px;text-align: center;width:70%;margin-left: -10%;">${locale === 'zh' ? "待确认环节" : "Unidenti fied"}</div>`
                    }
                  />
                </div>
                <div className="content" style={{ width: "25%", height: "235px" }}>
                  <MinLoopCharts
                    dataLink={dataList && dataList.byStatus[1]}
                    content={
                      `<div style="color:#666666;font-size:16px;text-align: center;width:70%;margin-left: -10%;">
                        ${locale === 'zh' ? "待处置环节" : "Undispos ed"}
                      </div>`
                    }
                  />
                </div>
                <div className="content" style={{ width: "25%", height: "235px" }}>
                  <MinLoopCharts
                    dataLink={dataList && dataList.byStatus[2]}
                    content={
                      `<div style="color:#666666;font-size:16px;text-align: center;width:70%;margin-left: -10%;">
                        ${locale === 'zh' ? "处置中环节" : "Dispos ing"}
                      </div>`
                    }
                  />
                </div>
                <div className="content" style={{ width: "25%", height: "235px" }}>
                  <MinLoopCharts
                    dataLink={dataList && dataList.byStatus[3]}
                    content={
                      `<div style="color:#666666;font-size:16px;text-align: center;width:70%;margin-left: -10%;">
                        ${locale === 'zh' ? "已处置环节" : "Dispos ed"}
                      </div>`
                    }
                  />
                </div>
            ...
            <div className="panel-wrap" style={{ width: "35%" }}>
              ...
              <div className="content">
                <Custom data={dataList && dataList.byAppCode} labelEng={locale === 'zh' ? "label" : "english"} />
              </div>
            </div>
          </div>
            ...
            <div style={{ display: "flex" }}>
              <div className="content" style={{ width: "80%" }}>
                <div className="tooltip">
                  <Histogram data={dataList && dataList.byOvertime} />
                </div>
              </div>
              <div className="histogram-right" style={{ marginLeft: "63px", marginTop: "5%" }}>
                <div style={{ marginBottom: "37px" }}>
                  ...
                  <div>
                    <div>
                      ...
                      {
                        dataList && dataList.byOvertime.toyearRate >= 0 ?
                          <Icon
                            type="arrow-up"
                            style={{
                              color: '#FC5B5B',
                      ...
                      <span>{dataList && dataList.byOvertime.toyearRate}%</span>
                    </div>
                    <div>
                      <span>
                      ...
                      {
                        dataList && dataList.byOvertime.tomonthRate >= 0 ?
                          <Icon type="arrow-up"
                            style={{
                              color: '#FC5B5B',
                              border: "1px solid #FC5B5B",
                      ...
                      <span>{dataList && dataList.byOvertime.tomonthRate}%</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <h4>{dataList && dataList.byOvertime.totalRate}%</h4>
                  <div>
                    <div>
                      <span>
                        <FormattedMessage id="event.YearOnYearGrowth" defaultMessage="Year-on-year Growth" />
                      ...
                      {
                        dataList && dataList.byOvertime.percentToyearRate >= 0 ?
                          <Icon
                            type="arrow-up"
                            style={{
                              color: '#FC5B5B',
                      ...
                      <span>{dataList && dataList.byOvertime.percentToyearRate}%</span>
                    </div>
                    <div>
                      <span>
                        <FormattedMessage id="event.chainGrowth" defaultMessage="Month-on-month Growth" />
                      ...
                      {
                        dataList && dataList.byOvertime.percentTomonthRate >= 0 ?
                          <Icon
                            type="arrow-up"
                            style={{
                              color: '#FC5B5B',
                      ...
                      <span>{dataList && dataList.byOvertime.percentTomonthRate}%</span>
                    </div>
                  </div>
                </div>
              </div>
