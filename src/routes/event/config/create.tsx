        ...
        this.props[request]({
          ...
          cb: (error: any) => {
            if (error.code === 10002 || error.code === 500) {
              ...
              message.success(this.id ? (locale === 'zh' ? '编辑成功' : 'Successful revision') : (locale === 'zh' ? '创建成功' : 'Create success'), 3);
  ...
  render() {
    ...
    return (
      <div className={classes.root}>
        <h3 className="title"><FormattedMessage id="event.eventPushConfiguration" defaultMessage="Event Push Configuration" /></h3>
        <Form layout={'horizontal'}>
          <Form.Item className={locale === 'zh' ? '' : 'labelForm'} label={<FormattedMessage id="event.sourceSystem" defaultMessage="Source System" />}{...formItemLayout}>
            {
              getFieldDecorator('appCode', {
                rules: [{ required: true, message: <FormattedMessage id="event.text.pleaseSelectYouBelong" defaultMessage="Please select the source system" /> }],
              })(
                <Select placeholder={<FormattedMessage id="event.text.pleaseSelectYouBelong" defaultMessage="Please select the source system" />} {...filterOption} getPopupContainer={triggerNode => triggerNode}
                >
                  {
          ...
          <Form.Item className={locale === 'zh' ? '' : 'labelForm'} label={<FormattedMessage id="event.eventType" defaultMessage="Event Type" />} {...formItemLayout}>
            {
              getFieldDecorator('typeId', {
                rules: [{ required: true, message: <FormattedMessage id="event.pleaseEnterEventType" defaultMessage="Please enter event type" /> }],
              })(
                <Select getPopupContainer={triggerNode => triggerNode}
                  placeholder={<FormattedMessage id="event.pleaseEnterEventType" defaultMessage="Please enter event type" />} {...filterOption}>
                  {
                    eventTypeList.map((item: any, index: number) => {
                      return <Option key={index} value={item.id}>{locale === 'zh' ? item.name : item.english}</Option>
                    })
                  }
                </Select>
              )
          ...
          <Form.Item className={locale === 'zh' ? '' : 'labelForm'} label={<FormattedMessage id="event.eventLevel" defaultMessage="Event Level" />} required {...formItemLayout}>
            {
              getFieldDecorator('level', {
                initialValue: [],
                rules: [{ required: true, message: <FormattedMessage id="event.text.pleaseSelectTheEventLevel" defaultMessage="Please select the event level" /> }],
              })(
                <Radio.Group>
                  {
          ...
          <Form.Item className={locale === 'zh' ? '' : 'labelForm'} label={<FormattedMessage id="event.notificationObject" defaultMessage="Notification Object" />} {...formItemLayout}>
            {
              getFieldDecorator('fwTargets', {
                rules: [{ required: true, message: <FormattedMessage id="event.text.pleaseSelectThePersonResponsible" defaultMessage="Please select the notification object" /> }],
              })(
                <Select
                  getPopupContainer={triggerNode => triggerNode}
                  placeholder={<FormattedMessage id="event.text.pleaseSelectThePersonResponsible" defaultMessage="Please select the notification object" />}
                  mode="multiple"
          ...
          >
            {getFieldDecorator("fwType", {
              initialValue: [],
              rules: [{ required: true, message: <FormattedMessage id="event.text.pleaseNotification" defaultMessage="Please select notification channel" /> }]
            })(
              <Checkbox.Group>
          ...
          <Form.Item className={locale === 'zh' ? '' : 'labelForm'} style={fwTypeChecked ? { display: 'block' } : { display: 'none' }} label={<FormattedMessage id="event.messageTemplate" defaultMessage="Message Template" />} {...formItemLayout}>
            {
              getFieldDecorator('fwTplText', {
                rules: [{
                  required: (fwTypeChecked ? true : false),
                  message: <FormattedMessage id="event.pleaseSelectTheMessageTemplate" defaultMessage="Please select the message template" />
                }],
              })(
                <Select placeholder={<FormattedMessage id="event.pleaseSelectTheMessageTemplate" defaultMessage="Please select the message template" />} {...filterOption}
                  getPopupContainer={triggerNode => triggerNode}
                  onChange={this.handleArea}
                >
                  {
                    eventTitleList.map((item: any, index: number) => {
                      return <Option value={item.id} key={index} >{locale === 'zh' ? item.tplTitle : item.english}</Option>
                    })
                  }
                </Select>
              )
...
const mapState2Props = ({ event: { appCodeList, eventTypeList, eventLevelList, forwardTypeList, peopleInfoList, eventTitleList }, intl: { locale } }: any) => ({
  appCodeList,
  eventTypeList,
  eventLevelList,
  forwardTypeList,
...
const mapDispatch2Props = ({
  event: { getAppCodeList, getEventTypeList, getForwardTypeList, getEventTitleList, getPeopleInfoList, getEventLevelList,
    setConfigDetail, getConfigDetail, setPeopleInfoList, editConfig, addConfig
  }
}: any) => ({
  getAppCodeList,
  getEventTypeList,
