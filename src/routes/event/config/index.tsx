      ...
      },
      {
        ...
        render: (text: string, record: any) => (
          <div className={classes.operate}>
            ...
            {
              ((this.isShowByResourcesLabel("event_get") && this.isShowByResourcesLabel("event_put")) || this.isShowByResourcesLabel("event_get")) && this.isShowByResourcesLabel("event_delete") ?
                (<Divider type="vertical" style={{ margin: 0 }} />)
                : null
            }
            {
              this.isShowByResourcesLabel("event_put") && (
                <Link to={`${match.path}/edit/${record.id}`}><FormattedMessage id="action.edit" defaultMessage="Edit" /></Link>
              )
            }
            {
      ...
      <div className={classes.root}>
        <div className={classes.create}>
          {this.isShowByResourcesLabel("event_create") && (
            <Button type="primary" onClick={this.goToCreate}><FormattedMessage id="action.createRules" defaultMessage="Create Rules" /></Button>
          )}
        </div>
        <div className={classes.searchBox}>
          <span className={classes.searchNav}><FormattedMessage id="event.eventLevel" defaultMessage="Event Level" />： </span>
          <Select
            value={level}
          ...
          <span className={classes.searchNav}><FormattedMessage id="event.sourceSystem" defaultMessage="sourceSystem" />：</span>
          <Select
            value={appCode}
            getPopupContainer={triggerNode => triggerNode}
            onChange={(value: any) => {
          ...
          >
            <Select.Option value="all" ><FormattedMessage id="event.allSourceSystem" defaultMessage="allSourceSystems" /></Select.Option>
            {
          ...
          <span className={classes.searchNav}><FormattedMessage id="event.notificationObject" defaultMessage="Notification Object" />：</span>
          <Select
            value={fwTargets}
            getPopupContainer={triggerNode => triggerNode}
            onChange={(value: any) => {
          ...
          >
            <Select.Option value="all" ><FormattedMessage id="event.allPersonResponsible" defaultMessage="allNotificationObject" /></Select.Option>
            {
          ...
          <br />
          <span className={classes.searchNav}><FormattedMessage id="event.notificationChannel" defaultMessage="notificationChannel" />： </span>
          <Select
            value={fwType}
            getPopupContainer={triggerNode => triggerNode}
            onChange={(value: any) => {
          ...
          >
            <Select.Option value="all" ><FormattedMessage id="event.allChannel" defaultMessage="allChannels" /></Select.Option>
            {
              forwardTypeList.map((item: any, index: number) => {
                return <Select.Option value={item.code + ''} key={index}>{locale === 'zh' ? item.label : item.english}</Select.Option>
              })
            }
          </Select>

          <span className={classes.searchNav}> <FormattedMessage id="event.eventType" defaultMessage="Event Type" />：</span>
          <Select
            value={typeId}
            getPopupContainer={triggerNode => triggerNode}
            onChange={(value: any) => {
          ...
          <span className={classes.searchNav}><FormattedMessage id="event.whetherPushIsEnabled" defaultMessage="Enable Push" />：</span>
          <Select
            value={fwEnabled}
            getPopupContainer={triggerNode => triggerNode}
            onChange={(value: any) => {
          ...
          >
            <Select.Option value="all"><FormattedMessage id="event.allPush" defaultMessage="allPush" /></Select.Option>
            <option value="0">{noPush}</option>
            <option value="1">{Push}</option>
          </Select>
          <Button type="primary" onClick={this.onSearch}><FormattedMessage id="action.search" defaultMessage="Search" /></Button>
        </div>
        <Table
          rowKey="id"
...
const mapState2Props = ({ event: { configListData, eventLevelList, forwardTypeList, eventTypeList, appCodeList, eventTitleList, peopleInfoList }, intl: { locale },
  app: { resourcesLabel },
...
const mapDispatch2Props = ({
  event: { getEventTypeList, getEventTitleList, getEventLevelList, getAppCodeList, editConfig, deleteConfig, getForwardTypeList, getConfigList, getPeopleInfoList, setPeopleInfoList },
  pageState: { putEventConfigPageState }
}: any) => ({
  getEventTypeList, getEventTitleList, getEventLevelList, getAppCodeList, editConfig, deleteConfig, getForwardTypeList, getConfigList, getPeopleInfoList, setPeopleInfoList, putEventConfigPageState
});
